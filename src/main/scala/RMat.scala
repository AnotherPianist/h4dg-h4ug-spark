...
class RMat {
  ...
  def generateEdge(n: Long): (Long, Long) = {
    ...
    offset3 = (m - c) / depth
    offset4 = (m - d) / depth
    this.chooseEdge(1, 1, n, n, a, b, c, d)
  }
  ...
  private def chooseEdge(x1: Long, y1: Long, xn: Long, yn: Long, a: Double, b: Double, c: Double, d: Double): (Long, Long) = {
    ...
    if (r < a)
      chooseEdge(x1, y1, math.floor((x1 + xn) / 2).toLong, math.floor((y1 + yn) / 2).toLong, newA, newB, newC, newD)
    else if (r >= a && r < ab)
      chooseEdge(math.ceil((x1 + xn) / 2).toLong, y1, xn, math.floor((y1 + yn) / 2).toLong, newA, newB, newC, newD)
    else if (r >= ab && r < abc)
      chooseEdge(x1, math.ceil((y1 + yn) / 2).toLong, math.floor((x1 + xn) / 2).toLong, yn, newA, newB, newC, newD)
    else
      chooseEdge(math.ceil((x1 + xn) / 2).toLong, math.ceil((y1 + yn) / 2).toLong, xn, yn, newA, newB, newC, newD)
  }
}